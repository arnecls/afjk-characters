...
nameHtml+"</span></a>");}
function renderInline(text){const parts=[];let last=0;const re=/`([^`]+)`|\[\[([^\]]+)\]\]/g;let match;while((match=re.exec(text))){const backtickLabel=match[1];const heroName=match[2];if(backtickLabel!==undefined){const merged=tryMergeTrailingLabel(text.slice(last,match.index),backtickLabel);if(merged){parts.push(merged);}else{parts.push(escapeHtml(text.slice(last,match.index)));parts.push(window.AFKJ.chips.formatTag(backtickLabel));}}else{parts.push(escapeHtml(text.slice(last,match.index)));parts.push(renderCharacterPill(heroName));}
