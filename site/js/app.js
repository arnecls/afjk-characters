...
function renderStatsOverviewRow(entries,withIcon){if(!entries||!entries.length){return"";}
return entries.map(function(entry){return chips.renderClassRankMergedPill(entry.label,entry.rank,"buff",withIcon);}).join("");}
function renderStatsOverview(statsOverview){if(!statsOverview){return"";}
const categories=renderStatsOverviewRow(statsOverview.categories,false);const stats=renderStatsOverviewRow(statsOverview.stats,true);if(!categories&&!stats){return"";}
