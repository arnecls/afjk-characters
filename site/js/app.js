...
function walkSpeedIndicatorMeta(value){const lower=String(value||"").trim().toLowerCase();if(!WALK_SPEED_STYLE[lower]){return null;}
return{cls:"chip-speed "+WALK_SPEED_STYLE[lower],label:lower,tooltip:WALK_SPEED_TOOLTIPS[lower],emoji:WALK_SPEED_EMOJI[lower],};}
function movementChipMeta(label){const trimmed=String(label||"").trim();if(!trimmed){return null;}
const lower=trimmed.toLowerCase();for(let i=0;i<MOVEMENT_KEYS.length;i++){const key=MOVEMENT_KEYS[i];if(lower===key.toLowerCase()){const def=MOVEMENT_DEFINITIONS[key];return{emoji:def.emoji,text:trimmed,cls:def.cls,};}}
return null;}
function mergeMovementWithWalkSpeed(movementLabel,walkSpeed){const walkMeta=walkSpeedIndicatorMeta(walkSpeed);if(!walkMeta){return null;}
const moveMeta=movementChipMeta(movementLabel);if(moveMeta){return formatMergedIndicator({hasIcon:true,emoji:moveMeta.emoji,text:moveMeta.text,cls:moveMeta.cls,tierSuffix:"",},walkMeta,false);}
return formatMergedIndicator({textOnly:movementLabel,tierSuffix:""},walkMeta,true);}
...
return false;}
function parseEffectCellPart(text){const segments=chips.splitSummarySegments(text);let effect="";let quality="";let conditional="";let timing="";if(segments.length){const leading=canonicalMergedEffectType(segments[0]);if(leading){effect=leading;segments.shift();}}
...
if(values.length&&values.every(function(value){return!!canonicalMergedEffectType(value);})){return values.slice().sort(function(a,b){return mergedEffectFilterSortRank(a,column)-
...
if(column==="Movement"){raw.split(/\s*\|\s*/).forEach(function(part){const trimmed=part.trim();if(trimmed){values.add(trimmed);}});return values;}
...
const parts=trimmed.split(/\s*\|\s*/);if(parts.length===2){const merged=chips.mergeMovementWithWalkSpeed(parts[0],parts[1]);if(merged){return merged;}}
...
return null;}
function expandLegacyFilterMap(filterMap){const expanded={};Object.keys(filterMap||{}).forEach(function(column){const values=filterMap[column]||[];const alias=legacyEffectColumnAlias(column);if(alias){if(!expanded[alias.column]){expanded[alias.column]=[];}
expanded[alias.column].push(alias.effect);if(!(values.length===1&&values[0]==="all")){values.forEach(function(value){if(value){expanded[alias.column].push(value);}});}
return;}
if(!expanded[column]){expanded[column]=[];}
values.forEach(function(value){expanded[column].push(value);});});Object.keys(expanded).forEach(function(column){const seen={};expanded[column]=expanded[column].filter(function(value){if(!value||seen[value]){return false;}
seen[value]=true;return true;});});return expanded;}
function applyListFilterMap(filterMap){const state=window.AFKJ.state;state.csvColumnFilters={};state.csvColumnFilterCombine={};const expanded=expandLegacyFilterMap(filterMap);Object.keys(expanded).forEach(function(column){const colIdx=columnIndex(column);if(colIdx===-1){return;}
const resolved=resolveFilterValues(column,colIdx,expanded[column]);if(!resolved.length){return;}
state.csvColumnFilters[colIdx]=resolved;if(column==="Behavior tags"&&expanded[column].length>1){state.csvColumnFilterCombine[colIdx]="and";}});state.viewMode="list";try{localStorage.setItem(window.AFKJ.config.VIEW_MODE_KEY,"list");}catch(e){}
...
let rest=match[1].trim();let walkSpeed="";const walkMatch=rest.match(/;\s*walk speed\s+(\S+)\s*$/i);if(walkMatch){walkSpeed=walkMatch[1].trim();rest=rest.slice(0,walkMatch.index).trim();}
const paren=rest.match(/^(.+?)\s*(\([^)]+\))\s*$/);const base=paren?paren[1].trim():rest;const suffix=paren?" "+escapeHtml(paren[2]):"";let chip=null;if(walkSpeed){chip=chips.mergeMovementWithWalkSpeed(base,walkSpeed);}
if(!chip){chip=formatMovementChip(base);}
return formatSkillOverviewRow("<strong>Movement</strong>",(chip!==null?chip:escapeHtml(base))+suffix);}
