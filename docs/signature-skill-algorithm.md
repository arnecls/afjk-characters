...
By default, the system calculates the "best repeatable, buffable skill"
(Ultimate, Skill 1, Skill 2, etc.). However, typed manual overrides in the
hero's `data/heroes/<hero-id>/overrides.json` are applied when a hero's true
identity lies elsewhere. The final result is the **Effective Signature Skill**.
